    ...
    <div className="relative h-full flex flex-col bg-card/85 border border-border rounded-[2.5rem] p-8 backdrop-blur-md group overflow-hidden shadow-xl transition-all duration-500 hover:border-blue-500/30">
      ...
      <div className="flex items-center justify-between mb-8 relative z-10">
        <div className="flex items-center gap-3">
          ...
          <span className="text-xs font-mono uppercase tracking-widest text-zinc-500 dark:text-zinc-400 font-bold">
            Tech Stack IQ
          </span>
        </div>
          ...
          <div key={lang.name} className="group/row">
            <div className="flex items-center justify-between mb-2 text-xs">
              <div className="flex items-center gap-3">
                ...
                />
                <span className="font-bold text-foreground/80 group-hover/row:text-blue-400 transition-colors uppercase tracking-[0.1em] text-[10px]">
                  {lang.name}
                </span>
              </div>
...
export function WakaTimeMetricPod({ label, value, subValue, icon, highlight, color, delay }: any) {
  return (
    <motion.div
      ...
      transition={{ delay }}
      className={`relative p-6 rounded-[2rem] border border-border bg-card/85 transition-all duration-500 group overflow-hidden h-full flex flex-col justify-between shadow-lg hover:shadow-2xl hover:shadow-blue-500/10 ${
        highlight
    ...
    >
      <div className="flex items-center gap-3 relative z-10">
        <div
          className={`p-2 rounded-lg bg-white/5 border border-white/5 group-hover:border-white/20 transition-colors ${color}`}
        >
          {icon}
        </div>
      ...
      <div className="mt-4 relative z-10">
        <div
          className={`text-3xl font-black tracking-tight ${highlight ? 'text-blue-500 dark:text-blue-400' : 'text-foreground'}`}
        >
          {value}
        </div>
        {subValue && (
      ...
      <div
        className={`absolute top-0 right-0 w-8 h-8 opacity-[0.03] group-hover:opacity-[0.1] transition-opacity ${highlight ? 'bg-blue-500' : 'bg-white'}`}
...
export default function GenericWakaTimeLoader({
  children,
  render,
}: {
