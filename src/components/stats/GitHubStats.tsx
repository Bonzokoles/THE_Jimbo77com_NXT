    ...
    const fetchGitHubData = async () => {
      try {
        ...
        if (res.ok) {
          ...
          const best =
            contributions.length > 0 ? Math.max(...contributions.map((c: any) => c.count)) : 0;
      ...
      <div className="mt-6 flex items-center justify-between text-[10px] font-mono text-muted-foreground uppercase tracking-[0.3em] font-bold">
        ...
        <div className="flex gap-1">
          {cyberpunkTheme.dark.map((c, i) => (
            <div
              key={i}
              className="w-2 h-2 rounded-full shadow-[0_0_5px_rgba(0,0,0,0.2)]"
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3 relative z-10">
        ...
        <span className="text-[10px] font-mono uppercase tracking-widest text-zinc-500 dark:text-zinc-400 font-bold group-hover:text-foreground transition-colors">
          {label}
        </span>
      </div>
      ...
      <div className="mt-4 relative z-10">
        <div className="text-3xl font-black tracking-tighter text-foreground group-hover:scale-105 transition-transform origin-left">
          {value}
          <span className="text-xs font-normal text-muted-foreground/50 ml-1">{suffix}</span>
        </div>
      </div>
