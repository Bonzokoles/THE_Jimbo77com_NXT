...
const CenterImage = ({ scrollY }: { scrollY: MotionValue<number> }) => {
  ...
  const opacity = useTransform(scrollY, [SECTION_HEIGHT + 1000, SECTION_HEIGHT + 1600], [1, 0]);
  ...
  return (
    <div className="sticky top-0 h-screen w-full flex items-center justify-center overflow-hidden z-0">
      ...
      >
        ...
        >
          Połączenie precyzji technicznej z kreatywną wizją. Moja droga zawodowa — od pierwszych
          linii kodu po zaawansowane rozwiązania AI.
        </motion.p>
      </motion.div>
    </div>
  );
...
const ParallaxImg = ({
  ...
  return <motion.img src={src} alt={alt} className={className} style={{ transform, opacity }} />;
};
