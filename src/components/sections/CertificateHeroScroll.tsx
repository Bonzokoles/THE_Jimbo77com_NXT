  ...
  const getPositions = useCallback((): Positions => {
    ...
    const desktopFinal: Record<string, Position> = {
      cert1: {
        ...
        left: startX,
        width: col1W,
        height: (gridH - gap) * 0.55,
        borderRadius: 8,
        zIndex: 10,
      },
      cert2: {
        top: startY + (gridH - gap) * 0.55 + gap,
        left: startX,
        width: col1W,
        height: (gridH - gap) * 0.45,
        borderRadius: 8,
        zIndex: 10,
      },
      cert3: {
        top: startY,
        left: startX + col1W + gap,
        width: col2W,
        height: (gridH - gap) * 0.4,
        borderRadius: 8,
        zIndex: 10,
      },
      cert4: {
        top: startY + (gridH - gap) * 0.4 + gap,
        left: startX + col1W + gap,
        width: col2W,
        height: (gridH - gap) * 0.6,
        borderRadius: 8,
        zIndex: 10,
      },
      cert5: {
        top: startY,
        left: startX + col1W + col2W + 2 * gap,
        width: col3W,
        height: (gridH - gap) * 0.65,
        borderRadius: 8,
        zIndex: 10,
      },
      cert6: {
        top: startY + (gridH - gap) * 0.65 + gap,
        left: startX + col1W + col2W + 2 * gap,
        width: col3W,
        height: (gridH - gap) * 0.35,
        borderRadius: 8,
        zIndex: 10,
      },
    };
  ...
  return (
    <>
      ...
      <div
        ...
        className="fixed inset-0 z-10 h-screen w-full overflow-hidden bg-transparent pointer-events-none"
      >
        {/* Background Effects */}
