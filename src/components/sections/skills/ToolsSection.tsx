...
export const ToolsSection = () => {
  ...
  return (
    <section className="py-32 relative bg-background min-h-screen flex flex-col items-center justify-center overflow-hidden">
      {/* BACKGROUND AMBIENCE */}
      ...
      >
        {portfolioData.tools.map((tool, index) => (
          <MistItem key={tool.name} tool={tool} mouseX={mouseX} mouseY={mouseY} index={index} />
        ))}
      </div>

      <div className="absolute bottom-12 text-center w-full pointer-events-none opacity-30 text-white/50">
        <p className="text-[10px] font-mono uppercase tracking-[0.4em]">[ Codzienny Arsenał ]</p>
      </div>
    </section>
  );
};
...
const MistItem = ({
  ...
  const iconUrl =
    toolLogos[tool.name] ||
    (tool.icon?.includes('http')
      ? tool.icon
      : `https://cdn.simpleicons.org/${tool.name.toLowerCase().replace(/[\s.]/g, '')}`);
