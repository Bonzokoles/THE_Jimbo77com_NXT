...
const renderRichText = (text: string) => {
  ...
  return parts.map((part, i) => {
    if (part.startsWith('**') && part.endsWith('**')) {
      return (
        <strong key={i} className="font-bold text-foreground bg-primary/10 px-1 rounded mx-0.5">
          {part.slice(2, -2)}
        </strong>
      );
    }
...
export function ProjectDetail({ project, onClose }: { project: Project; onClose: () => void }) {
  ...
  const [activeSection, setActiveSection] = useState<
    'overview' | 'tech' | 'features' | 'gallery' | 'install'
  >('overview');
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div
          ref={heroRef}
          className="relative lg:w-5/12 h-[35vh] lg:h-full flex flex-col justify-end p-6 sm:p-10 overflow-hidden group cursor-pointer"
                ...
                >
                  ...
                  <div className="space-y-10">
                    ...
                    <motion.div
                      ...
                      className="flex flex-wrap justify-center gap-x-16 gap-y-8 py-10 border-y border-black/5 dark:border-white/5"
                    >
                      {[
                        ...
                        {
                          label: 'Status',
                          value: isOngoing ? 'Aktywny' : 'Ukończony',
                          icon: CheckCircle2,
                        },
                      ].map((item, i) => (
                          ...
                          <div key={idx} className="relative group">
                            ...
                            <div className="space-y-4">
                              ...
                              <div className="text-sm text-muted-foreground leading-relaxed pl-4 border-l border-black/5 dark:border-white/5 group-hover:border-emerald-500/50 transition-colors">
                                <span className="text-emerald-600 dark:text-emerald-500 font-bold text-xs uppercase tracking-wider block mb-1">
                                  Rozwiązanie
                                </span>
                                {item.solution}
                              </div>
                        ...
                        >
                          ...
                          <div className="space-y-4 relative z-10 font-light">
                            {group.items.map((item, i) => (
                              <div
                                key={i}
                                className="flex gap-3 text-sm text-muted-foreground group-hover:text-foreground transition-colors leading-relaxed"
                              >
                ...
                >
                  {project.installation ? (
                    project.installation.map((step, i) => (
                      <div
                        key={i}
                        className="bg-black/5 dark:bg-white/5 rounded-xl overflow-hidden border border-black/10 dark:border-white/5"
                      >
