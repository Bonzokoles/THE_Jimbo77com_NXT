...
export const OrbitalWorkbench = ({ tools, className }: OrbitalWorkbenchProps) => {
  ...
  return (
    <div
      ref={containerRef}
      className={`relative min-h-[250vh] py-64 flex items-center justify-center bg-black overflow-hidden ${className}`}
    >
      ...
      <div className="absolute inset-0 z-0 pointer-events-none">
        ...
        <motion.div
          ...
          style={{
            backgroundImage: 'radial-gradient(circle, transparent 40%, #111 70%, black 100%)',
          }}
        />
      </div>
      ...
      <div className="sticky top-0 h-screen w-full flex items-center justify-center overflow-hidden transform-gpu">
        ...
        <div className="relative z-10 flex flex-col items-center will-change-transform">
          ...
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-center pointer-events-none">
            <span className="block text-[8px] font-black uppercase tracking-[1.5em] text-primary/30">
              Catalyst
            </span>
            <h2 className="text-4xl font-black italic uppercase tracking-tighter text-white/5 select-none">
              ORBITAL
            </h2>
          </div>
        </div>
        ...
        >
          {orbits.map((ring, ringIdx) => (
            <OrbitalRing key={ringIdx} ring={ring} progress={smoothProgress} index={ringIdx} />
          ))}
        </div>
      </div>
          ...
          <div className="relative flex flex-col items-center">
            <motion.div style={{ rotate: counterRotate }}>
              ...
              <div className="absolute -bottom-16 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-all duration-300 translate-y-4 group-hover:translate-y-0 w-32 text-center pointer-events-none">
                <span className="block text-[8px] font-black uppercase tracking-[0.5em] text-primary mb-1">
                  {tool.name}
                </span>
