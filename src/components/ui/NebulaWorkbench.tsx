...
export const NebulaWorkbench = ({ tools, className }: NebulaWorkbenchProps) => {
  ...
  return (
    <div
      ref={containerRef}
      className={`relative min-h-[150vh] flex flex-col items-center justify-center py-48 overflow-hidden ${className}`}
    >
      ...
      >
        <span className="text-[10px] font-black uppercase tracking-[1em] text-primary/30">
          Artisan_Workbench
        </span>
        <h2 className="text-5xl md:text-7xl font-black italic uppercase tracking-tighter mt-4 text-white/10">
          Nebula_01
        </h2>
      </motion.div>
            ...
            >
              <div className="relative w-48 h-48 flex items-center justify-center">
                ...
                <div className="absolute -bottom-12 opacity-0 group-hover:opacity-100 transition-all duration-700 translate-y-4 group-hover:translate-y-0 text-center">
                  <span className="block text-xs font-black italic uppercase tracking-widest text-primary">
                    {tool.name}
                  </span>
                  <span className="block text-[8px] font-mono text-white/20 mt-1">
                    CORE_ARTIFACT_{idx}
                  </span>
                </div>
              </div>
