...
export default function GalleryPage() {
  ...
  const nextMedia = () =>
    setActiveMedia((prev) => (prev !== null ? (prev + 1) % filteredMedia.length : null));
  const prevMedia = () =>
    setActiveMedia((prev) =>
        ...
        <header className="mb-20">
          <div className="flex items-center gap-6 mb-8">
            <div className="flex flex-col">
              <span className="text-[10px] font-mono uppercase tracking-[0.8em] text-primary font-black opacity-40">
                VISUAL_UNIT
              </span>
              <span className="text-[10px] font-mono uppercase tracking-[0.8em] text-primary/60 font-black">
                ARCHIVE_v2
              </span>
            </div>
              ...
              >
                <div className="relative overflow-hidden aspect-auto">
                  ...
                  <div className="absolute bottom-6 left-6 right-6 opacity-0 group-hover:opacity-100 transition-all duration-500 translate-y-[10px] group-hover:translate-y-0">
                    <p className="text-[10px] font-mono text-primary uppercase tracking-[0.2em] mb-2">
                      {item.category}
                    </p>
        ...
        {activeMedia !== null && (
          ...
          >
            <button
              ...
              className="absolute top-8 right-8 p-3 bg-foreground/5 dark:bg-white/5 hover:bg-foreground/10 dark:hover:bg-white/10 rounded-full transition-colors z-[110] border border-foreground/10 dark:border-white/10"
            >
            ...
            <button
              onClick={prevMedia}
              className="absolute left-8 top-1/2 -translate-y-1/2 p-3 bg-foreground/5 dark:bg-white/5 hover:bg-foreground/10 dark:hover:bg-white/10 rounded-full transition-colors z-[110] border border-foreground/10 dark:border-white/10 hidden md:block"
            >
            ...
            <button
              onClick={nextMedia}
              className="absolute right-8 top-1/2 -translate-y-1/2 p-3 bg-foreground/5 dark:bg-white/5 hover:bg-foreground/10 dark:hover:bg-white/10 rounded-full transition-colors z-[110] border border-foreground/10 dark:border-white/10 hidden md:block"
            >
