        ...
        <article className="bg-secondary/[0.03] backdrop-blur-xl border border-border/40 rounded-3xl p-8 md:p-16 shadow-2xl">
          ...
          <div className="mt-20 pt-12 border-t border-border/40 flex flex-wrap gap-4">
            {post.tags.map((tag) => (
              <span
                key={tag}
                className="px-6 py-2 bg-secondary/10 border border-border/40 rounded-full text-[10px] font-black uppercase tracking-widest text-muted-foreground hover:text-primary hover:border-primary/40 transition-all cursor-default"
              >
                #{tag}
              </span>
            ))}
