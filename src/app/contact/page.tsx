    ...
    >
      ...
      <div className="relative z-10 flex items-center gap-4">
        ...
        <div className="flex flex-col">
          <span className="text-lg font-bold text-foreground group-hover:text-primary transition-colors tracking-tight">
            {item.name}
          </span>
            ...
            >
              <span
                className={cn(
                  ...
                  openIndex === index
                    ...
                    : 'text-foreground/90 group-hover:text-foreground group-hover:translate-x-1'
                )}
              >
                {faq.q}
              </span>
              <div
                className={cn(
                  ...
                  openIndex === index
                    ? 'rotate-180 bg-primary text-primary-foreground border-primary shadow-[0_0_20px_rgba(var(--primary-rgb),0.3)]'
                    : 'text-muted-foreground group-hover:text-foreground group-hover:bg-primary/10'
                )}
              >
...
export default function ContactPage() {
  ...
  const getSocialItem = (platform: string) => {
    const link = portfolioData.personal.socialLinks.find(
      (l) => l.platform.toLowerCase() === platform
    );
    return {
      name:
        (link?.platform || platform).charAt(0).toUpperCase() +
        (link?.platform || platform).slice(1),
