          ...
          >
            <div
              className={cn(
                ...
                isOngoing
                  ? 'border-emerald-500/20 bg-emerald-500/5 dark:bg-emerald-950/80'
                  : 'border-blue-500/20 bg-blue-500/5 dark:bg-blue-950/80'
              )}
            >
              {project.image ? (
                <img
                ...
                <div className="w-full h-full flex items-center justify-center bg-zinc-900/50">
                  <span
                    className={cn(
                      'text-6xl font-black',
                      isOngoing ? 'text-emerald-400/20' : 'text-blue-400/20'
                    )}
                  >
        ...
        <div className="relative h-full bg-white/50 dark:bg-black/40 backdrop-blur-2xl rounded-3xl overflow-hidden border border-black/5 dark:border-white/10 transition-colors duration-500">
          ...
          <div className="absolute bottom-0 left-0 right-0 p-8 sm:p-10 md:p-14 z-30">
            ...
            >
              ...
              >
                <Sparkles
                  className={cn('w-5 h-5', isOngoing ? 'text-emerald-400' : 'text-blue-400')}
                />
              </motion.div>
              <span
                className={cn(
    ...
    >
      <motion.div
        ...
        whileHover={{
          y: -4,
          scale: 1.02,
          borderColor: isOngoing ? 'rgba(16, 185, 129, 0.3)' : 'rgba(59, 130, 246, 0.3)',
        }}
...
const getIconKey = (name: string): keyof typeof Icons => {
  ...
  return (
    (Object.keys(Icons).find((k) => lower.includes(k.toLowerCase())) as keyof typeof Icons) ||
    'unknown'
  );
};
...
export default function ProjectsPage() {
    ...
    if (selectedCategory !== 'All') {
      if (selectedCategory === 'More') {
        projects = projects.filter(
          (p) =>
            p.category && ['IoT & Embedded', 'Blockchain', 'Creative Tech'].includes(p.category)
        );
      } else {
  ...
  return (
    ...
    >
        ...
        {filteredProjects.length > 10 && (
          ...
          >
            <button
              onClick={() =>
                setVisibleCount(
                  visibleCount < filteredProjects.length ? filteredProjects.length : 10
                )
              }
              ...
              <span className="relative z-10 flex items-center gap-2">
                ...
                <ArrowRight
                  className={cn(
                    ...
                    visibleCount >= filteredProjects.length && 'rotate-180'
                  )}
                />
              </span>
      ...
      <AnimatePresence>
        {selectedProject && <ProjectDetail project={selectedProject} onClose={handleCloseModal} />}
      </AnimatePresence>
    </div>
  );
}
